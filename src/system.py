...
class System:
    ...
    def __init__(self, admin_privileges_required=False):
        self.__has_admin_permissions = False
        if admin_privileges_required:
            self.get_admin_privileges()

    # This asks the user for admin privileges
    def get_admin_privileges(self):
        ...
        if sys.argv[-1] == ASADMIN:
            self.__has_admin_permissions = True

        return False

    # this can be used so if your program has dependencies you can easily use this function to install required libraries for your program!
    ...
    def install_package(self, libname: str):
        if self.__has_admin_permissions:
